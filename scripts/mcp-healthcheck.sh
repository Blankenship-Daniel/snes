...
if [[ $SKIP_BSNES_PLUS -eq 1 ]]; then
...
elif [[ -d repos/bsnes-plus ]]; then
  log "- bsnes-plus: FOUND"
  comp_err=0; comp_warn=0; comp_last=""
  # Component fingerprint (last commit touching this dir)
  if command_exists git; then
    FP=$(git log -1 --format='%h %cd' --date=short -- repos/bsnes-plus 2>/dev/null || true)
    if [[ -n "$FP" ]]; then
      log "  * last-change: $FP"; comp_last="$FP"
    fi
  fi
  if [[ -f repos/bsnes-plus/README.md ]]; then
    log "  * README.md: OK"
  else
    log "  * README.md: MISSING"; FAILURES=$((FAILURES+1)); comp_err=$((comp_err+1))
  fi
  if [[ -f repos/bsnes-plus/bsnes/ui-qt/debugger/debugger.cpp ]]; then
    if search "Breakpoint" repos/bsnes-plus/bsnes/ui-qt/debugger/debugger.cpp; then
  ...
  if [[ -f repos/bsnes-plus/bsnes/Makefile && -f repos/bsnes-plus/snesreader/Makefile ]]; then
...
elif [[ -d repos/snes9x ]]; then
  log "- snes9x: FOUND"
  comp_err=0; comp_warn=0; comp_last=""
  if command_exists git; then
    FP=$(git log -1 --format='%h %cd' --date=short -- repos/snes9x 2>/dev/null || true)
    if [[ -n "$FP" ]]; then
      log "  * last-change: $FP"; comp_last="$FP"
    fi
  fi
  if search "void S9xMainLoop" repos/snes9x; then
    log "  * S9xMainLoop symbol found: OK"
  else
    log "  * S9xMainLoop symbol found: MISSING"; FAILURES=$((FAILURES+1)); comp_err=$((comp_err+1))
  fi
  if [[ -f repos/snes9x/cpuexec.cpp ]]; then
    if search "S9xMainLoop" repos/snes9x/cpuexec.cpp; then
...
if [[ $SKIP_ZELDA3 -eq 1 ]]; then
...
elif [[ -d repos/zelda3 ]]; then
  log "- zelda3: FOUND"
  comp_err=0; comp_warn=0; comp_last=""
  if command_exists git; then
    FP=$(git log -1 --format='%h %cd' --date=short -- repos/zelda3 2>/dev/null || true)
    if [[ -n "$FP" ]]; then
      log "  * last-change: $FP"; comp_last="$FP"
    fi
  fi
  SEARCH_DIR="repos/zelda3/src"
  [[ -d "$SEARCH_DIR" ]] || SEARCH_DIR="repos/zelda3"
  ...
  if [[ -f repos/zelda3/src/sprite.c && -f repos/zelda3/src/sprite_main.c ]]; then
...
elif [[ -d repos/SNES_MiSTer ]]; then
  log "- SNES_MiSTer: FOUND"
  comp_err=0; comp_warn=0; comp_last=""
  if command_exists git; then
    FP=$(git log -1 --format='%h %cd' --date=short -- repos/SNES_MiSTer 2>/dev/null || true)
    if [[ -n "$FP" ]]; then
      log "  * last-change: $FP"; comp_last="$FP"
    fi
  fi
  if [[ -f repos/SNES_MiSTer/SNES.qpf || -f repos/SNES_MiSTer/SNES_Q13.qpf ]]; then
    log "  * Quartus project file present: OK"
  else
    log "  * Quartus project file present: MISSING"; FAILURES=$((FAILURES+1)); comp_err=$((comp_err+1))
  fi
  if [[ -d repos/SNES_MiSTer/rtl ]]; then
    log "  * rtl/ directory present: OK"
    # Check for core RTL files
    if [[ -f repos/SNES_MiSTer/rtl/SNES.vhd || -f repos/SNES_MiSTer/rtl/SNES.v ]]; then
      log "  * SNES top RTL file present: OK"
    else
      log "  * SNES top RTL file present: MISSING"; FAILURES=$((FAILURES+1)); comp_err=$((comp_err+1))
    fi
    if [[ -f repos/SNES_MiSTer/rtl/P65C816.vhd ]]; then

...
export const DiscoveryId = (type: string, offset: number): DiscoveryId => {
  return `${type}:0x${offset.toString(16).padStart(6, '0')}:${Date.now()}`;
};
...
export class DiscoveryBuilder<T extends Discovery['type']> {
  private discovery: any; // Simplified to avoid complex generic issues
  ...
  offset(value: number): this {
    ...
    (this.discovery).offset = ROMOffset(value);
    return this;
  }
  ...
  size(value: number): this {
    ...
    (this.discovery).size = ByteSize(value);
    return this;
  }
  
  confidence(level: Extract<Discovery, { type: T }>['confidence']): this {
    (this.discovery).confidence = level;
    return this;
  }
  ...
  metadata(meta: DiscoveryMetadata): this {
    ...
    (this.discovery).metadata = meta;
    return this;
  }
  
  data(data: Extract<Discovery, { type: T }>['data']): this {
    (this.discovery).data = data;
    return this;
  }
  ...
  build(): Extract<Discovery, { type: T }> {
    ...
    // Use double assertion through unknown for complex generic type
    return this.discovery as unknown as Extract<Discovery, { type: T }>;
  }
}